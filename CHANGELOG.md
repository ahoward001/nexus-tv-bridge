...
**"NET 2.3m" was a bad label.** It was honest — the span really was 2.3 minutes, because
the samples land wherever the syncs land — but a label has to be readable at a glance, and
that one raised more questions than the number answered. The row is **MOM** now, and the
span moved to the hover: *"Net delta flow over the last 2.3 minutes."* FLOW gets a hover
too: *"Cumulative net delta flow so far today."*

What it measures hasn't changed. Nexus publishes only a 1-minute momentum and no window
control, so MOM is the change in cumulative net delta flow between two readings — which is
exactly the net flow over that gap.

**The gravity row is off the panel.** Gravity is drawn on the chart now, which is where a
price level belongs. Keeping a copy on the panel meant a row reading "—" whenever the
cached value was empty: a blank taking up space to tell you nothing.

It also **stopped fetching gravity when the panel expands**. That request had nowhere to
land any more, and on a cold cache it opens a Nexus tab — work, and a flash of a tab, for
a row that no longer exists. The chart's copy still refreshes on the background schedule.
...
GEX tells you how hard dealers have to defend a strike. DEX tells you which way they are
already leaning into it, and it was the one per-strike figure the board carried that the
chart didn't. It reads exactly like GEX — green long, red short, intensity by size against
the board — and ships off by default, so nothing changes until you tick it.

The two newest columns were also only half-finished:

- **% of board** was scaled against a flat 100. The top strike on a normal board is 15-25%,
  so nothing ever reached half intensity and the whole column read uniformly pale. It now
  scales against the biggest share actually on the board, so the leader is solid and the
  rest rank honestly underneath it.
- **Net vol** claimed no scale at all — every strike printed at the same mid-tone, which
  meant the column ranked nothing. It now carries the same session-anchored heat as OI and
  GEX, so a strike that came alive today is visibly louder than one that ticked.

**% of board sits closer to the block now.** It holds "17%" where its neighbours hold
"-29.4M", so identical bar spacing left a wider lane after it and the column looked
detached. Nudged right by a fifth of a gap — the same fix Score already had, and for the
same reason: it corrects a width artefact, not a preference.

**In the panel, GRAD and ΔDEX lost their units and gained colour.** Every GRAD reading was
"M/min" and every DEX reading was "M" or "B", so the unit was never the thing that changed
— and the mixed suffixes made readings non-comparable at a glance: "-605.8M" next to
"-2.3B" is a 4x difference that reads like a 265x one. DEX is now normalised to millions
so the digits alone rank correctly, and the sign moved into the colour.
...
If you have ever ended up with two saved copies of the script — TradingView adds a number, so `Nexus Strike Metrics` and `Nexus Strike Metrics 1` — the extension could not tell them apart. Both match the name it looks for, so it would open whichever the Pine editor happened to have, paste the new source into it, save it, verify it, and report success. All of that was true. None of it touched the script your chart is actually running.

That is why the three new columns never appeared no matter how many times the update ran: the columns were being added to a script nothing was using.

It now compares the script in the editor against the one named in your chart's legend, and refuses to write when they differ — telling you which two names it found rather than succeeding at the wrong thing.

**If this is you, the fix is to delete the spare.** Pine Editor → the script-name dropdown → My scripts → remove the copy with the number on the end. With one script left there is nothing to get wrong, and the next sync will update it properly.

A silent success on the wrong target is the worst failure mode this extension has had. Everything it reported was accurate and the outcome was still nothing.
...
**The panel's expander did nothing. Now it works.**

The three readings it was meant to reveal — GEX gradient, net delta flow, momentum — were being read by the wrong half of the extension. They came from a function whose result the panel never receives, so every extra row was simply absent and expanding opened an empty half. The arrow toggled and nothing appeared.

They're now read where the panel can actually see them.

The underlying mistake is one worth naming, because it has caused three separate bugs this month: the parts of this extension that run inside the dashboard page are each shipped over there **on their own**, so a helper written in one is invisible to the others even though they sit in the same file. Each needs its own copy. That's what broke the strike columns two weeks ago and it's what broke this.
...
The strike read used a single tab — whichever the export code happened to come from — and accepted whatever it returned. If that tab was parked on a different ticker, mid-load, or being poked at, the sync either reported *"the strike read returned nothing"* or drew nonsense, while a perfectly good tab on the right ticker sat open beside it.

Seen this morning: a read **centred on 7689.76 while QQQ was 717**. That isn't a stale snapshot, it's a different instrument — and the extension had no notion of trying somewhere else.

It now works through every open Nexus tab: the one it just read the export from first, then any already on the right ticker, then the rest. A snapshot centred more than 15% away from price is rejected as "not this instrument" and it moves on rather than giving up. The log names the tab it succeeded with and why the others were skipped.

That threshold is deliberately coarse — it catches the wrong page, not a slightly old one. The existing staleness rules still decide whether a plausible read is fresh enough to draw.

**Also worth knowing:** the dashboard's on-screen strike grid no longer exists on the overview page as a readable table — checked live today, the page carries no grid rows at all where it used to have twenty-seven. The strike data now comes from the copy embedded in the page, which is intact and correct. That fallback carrying its own weight is why the columns still fill at all.
...
**When a new version lands, the button now tells you to reload the tab.**

Updating the extension — the Store pushing a build, or reloading the unpacked copy — swaps it underneath every chart tab you already have open. The scripts already running in those tabs keep running the **old** code, detached from the extension that replaced them. The button looks completely healthy and does nothing at all when clicked, because every message it sends throws.

Nothing on the page told you this. The button now goes **yellow**, the hover says *"A new version of the extension was installed — this tab is still running the old one"*, and clicking reloads the tab, which is the only thing that actually fixes it.

This is also why a fix could look like it hadn't shipped: you'd reload the extension, click sync on a chart tab you'd had open the whole time, and get the previous build's behaviour.

*(No new permissions for this. It works by noticing the extension is no longer reachable from the page, which needs nothing beyond what's already granted — a version check against GitHub would have required a new permission and prompted everyone on update.)*
...
v5.1.3.1 stopped two syncs from running at once, which fixed a real problem: overlapping runs fought over the Nexus tab and one would recover by opening a stray tab. But the lock had no way out. If a run never finished — and a throttled or wedged dashboard tab is enough to cause that — the extension went on believing a sync was still in progress, and **every click after that was silently ignored**. Seen today: a run gave up waiting on a loading tab, then seven clicks in a row were dropped with "already running". The only cure was waiting for the browser to restart the extension.

The lock now expires. If a run has been going for more than a minute, the next click assumes it's stuck and starts fresh — and the abandoned run can't come back later and interfere with the one that replaced it.

That was a fix of mine creating a worse problem than the one it solved: overlapping syncs were untidy, but a button that stops responding is useless.
...
## v5.1.13.1 — 2026-09-01

**One half failing no longer cancels the other half.**

A sync does two independent jobs from two different places on the dashboard: it reads the export code to set your levels, and it reads the strike table to fill the Score / OI / GEX columns. Failing to read one says nothing about whether the other is readable.

The code didn't treat them that way. If the export code couldn't be read, the run **stopped right there** — the columns were never even attempted, no matter how readable they were, and the message blamed the missing export code for everything. Now it tries the other job first and tells you which half actually worked: *"Levels didn't update — columns did."*

**And the failure messages now name the real cause.** "Couldn't read the strike data" was the same sentence whether the table hadn't rendered, the page carried no strike data at all, or — for the last two releases — the reader crashed on its first line and never read anything. It now says which of those happened, and makes clear the levels themselves applied fine.

A message that says something is unreadable should mean it was read and found wanting. Several of these were reporting a broken check as a broken page.
...
## v5.1.12.1 — 2026-09-01

**"Strike metrics didn't update" was my fault, introduced two releases ago.**

v5.1.8.1 added a helper that reads page text without depending on the browser having laid the page out. I called it from inside the strike reader — but that reader is serialised and injected into the page, so helpers defined elsewhere in the extension don't exist inside it. It threw on its first line, the error handler turned that into "no data", and every sync since reported *"Couldn't read the strike data, so the Score / OI / GEX columns are stale"* while the levels applied normally.

Nothing was ever unreadable. The reader was dead before it looked at anything. It now carries its own copy of that helper, like every other local it needs, and every caller has been checked for the same mistake.

**Separately: "in-place skipped — tab is fully loaded with no export panel" was also wrong.** That check looked for an element the dashboard redesign removed, so it reported "no export panel" on every call — including on a tab sitting on the export view. The cost wasn't just a misleading line in the log: the extension concluded the tab could never answer and did a **full reload on every sync**, when it could often have read the page it already had.

It now recognises the export view by something that still exists — its loading bar, or the code box itself. Syncs should need fewer reloads.

If a message tells you something is unreadable, it should mean the thing was read and found wanting. Both of these were reporting a broken check as a broken page.
...
After a sync reloads the dashboard, the extension reads the per-strike table to fill the Score / OI / GEX columns. That read looked exactly once: if the page hadn't finished putting its data in place yet, it returned nothing and you got *"Couldn't read the strike data, so the Score / OI / GEX columns are stale. Levels themselves did apply."* — on a page that had the data a moment later. Losing that race is why the warning arrived almost instantly rather than after any real wait.

It now waits for the data to exist before reading, up to eight seconds, and checks both places it can live. Which one is available depends on the view: the on-screen strike grid exists on Overview but **not** on Export — verified live today, zero rows there — while the payload embedded in the page is present on both. A sync leaves the tab on Export, so the payload is usually what it reads.

Both sources were verified working against the live dashboard while writing this: 26 rows off the grid, 25 out of the payload, with matching strikes and values. The parsing was never the problem; the timing was.
